...
class Invoices extends APP_Controller
{
	...
	public function datatable()
	{
		...
		{
			$columns    	= "invoiceId,number,amount_total,statusId,status_name,class";
	...
	public function edit($invoiceId)
	{
		...
		$data['table']            	= $this->invoices_items_model->get_all_items($invoiceId,'invoiceId');
		$data['parents']			= $this->padres_madres();
		$data['number']				= $this->invoices_model->get_last_number($this->schoolId);
		$data['status'] 			= $this->invoices_status_model->get_assoc_list(array("statusId AS id", "name"), array("hidden" => 0), TRUE, FALSE);
	...
	{
		$data['row']            	= $this->invoices_model->get($invoiceId);
		$data['client']            	= $this->invoices_model->get_data_print($invoiceId);
		$data['table']            	= $this->invoices_items_model->get_all_items($invoiceId,'invoiceId');
		$data['number']				= $this->invoices_model->get_last_number($this->schoolId);
		$data['content']			= 'invoices/invoice_preview_view_original';
		// print_d($data['row']);
		$this->load->view('invoices/invoice_preview_view_original', $data);
	}

	private function padres_madres()
	{
		$result = $this->student_family_model->get_assoc_list(array('parentId AS id', 'full_name as name'), array('schoolId' => $this->schoolId, 'hidden' => 0),TRUE);
	...
	public function remove_items($itemId)
	{
		$items = array(
			'hidden' 		=> 1,
		);
